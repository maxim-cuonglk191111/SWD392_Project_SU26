 ...
 * Must mirror backend/src/services/anonymousService.ts exactly
 */
...
export function getAnonymousAvatarStyle(seed: number): { hue: number; bg: string } {
  const hue = (seed * 37) % 360;
  return { hue, bg: `hsl(${hue}, 65%, 55%)` };
}
